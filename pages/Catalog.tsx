...
const Catalog = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(productRef, (snapshot) => {
      ...
      // Changed to warn to avoid alarming console errors when running in restricted mode
  ...
  const getImageUrl = (url?: string) => {
    ...
    // If it's a data URI (uploaded via admin), return as is
    if (url.startsWith('data:') || url.startsWith('blob:')) return url;
    
    // If it's Unsplash, we can safely optimize
    if (url.includes('images.unsplash.com')) {
       return `${url}&w=400&q=80`;
    }
    
    // Return original URL for everything else to avoid breaking standard links
    return url;
  };
